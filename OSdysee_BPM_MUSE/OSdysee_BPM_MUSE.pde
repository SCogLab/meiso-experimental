...
void setup() {
  ...
  header = "HrVal;museSampleSec;museSampleMS;accX;accY;accZ;";
...
{  
  ...
  if (theOscMessage.addrPattern().equals("/muse/eeg") == true) {
    //print("OSC Message Recieved: ");
    //print(theOscMessage.addrPattern() + " ");
    museSampleSec = theOscMessage.get(4).intValue();
    museSampleMsec = theOscMessage.get(5).intValue();
    //println(museSampleSec + " " + museSampleMsec);
    //theOscMessage.print();
  } else if (theOscMessage.addrPattern().equals("/muse/acc") == true){
    for (int i=0; i<3; i++){
      acc[i] = theOscMessage.get(i).floatValue();
    }
  } else if (theOscMessage.addrPattern().equals("/muse/elements/alpha_absolute") == true) {
    for (int i=0; i<numChannels; i++) {
      alpha[i] = theOscMessage.get(i).floatValue();
    }
  } else if (theOscMessage.addrPattern().equals("/muse/elements/beta_absolute") == true) {
    for (int i=0; i<numChannels; i++) {
      beta[i] = theOscMessage.get(i).floatValue();
    }
  } else if (theOscMessage.addrPattern().equals("/muse/elements/delta_absolute") == true) {
    for (int i=0; i<numChannels; i++) {
      delta[i] = theOscMessage.get(i).floatValue();
    }
  } else if (theOscMessage.addrPattern().equals("/muse/elements/theta_absolute") == true) {
    for (int i=0; i<numChannels; i++) {
      theta[i] = theOscMessage.get(i).floatValue();
    }
  } else if (theOscMessage.addrPattern().equals("/muse/elements/gamma_absolute") == true) {
    for (int i=0; i<numChannels; i++) {
      gamma[i] = theOscMessage.get(i).floatValue();
    }
  } else if (theOscMessage.addrPattern().equals("/muse/config/eeg_channel_layout") == true) {
    theOscMessage.print();
  }
}
...
void draw() {
  if (curInitState == 0) {
    ...
    chan_data = "";
    for (int i=0; i<numChannels; i++) {
      chan_data = chan_data + ";" + delta[i] + ";" + theta[i] + ";" + alpha[i] + ";" + beta[i] + ";" + gamma[i];
    }
    acc_data = "";
    for (int i=0; i<3; i++){
